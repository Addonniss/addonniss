...
def check_for_subs(self):
    ...
    for f in candidate_files:
        ...
        # Snapshot the current size BEFORE processing so that on the next
        # poll this file is not re-entered even if process_subtitles loaded
        # an already-translated cached copy without returning True.
        ...
        if not success:
            # Roll back the snapshot so a failed attempt is retried next poll.
...
```
    ...
    while not monitor.abortRequested():
        ...
        # Use a long sleep interval when nothing is playing to avoid
        # waking up every 3 seconds for no reason. Switch to the short
        # interval only during active playback.
