...
def get_version(xml_path):
    try:
        with open(xml_path, 'r', encoding='utf-8') as f:
            ...
            if match:
                return str(match.group(1)).strip() # Force it to stay as text
            return "1.0.0"
    except:
        return "1.0.0"
...
def create_repo():
    ...
    for addon_id in [service_id, repo_id]:
        ...
        # FETCH VERSION AS PURE STRING
        version_str = get_version(xml_path)
        print(f"Processing {addon_id} as Version: {version_str}")
        ...
        zip_name = f"{addon_id}-{version_str}.zip"
